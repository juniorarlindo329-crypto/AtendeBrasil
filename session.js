(function(){
  ...
  function getAccessToken(){ return localStorage.getItem(ACCESS) || sessionStorage.getItem(ACCESS); }
  ...
  async function refreshSession(){
    ...
    }catch(e){ console.error("Falha ao renovar sessão",e); return null; }
  }
  ...
  async function validToken(){
    ...
    if(exp && Date.now() >= exp) token = await refreshSession();
    return token;
  }
  ...
  async function authFetch(url, options={}){
    ...
    const headers = {"apikey":C.SUPABASE_KEY,"Authorization":`Bearer ${token}`,...(options.headers||{})};
  ...
  window.AtendeSession={saveSession,clearSession,getAccessToken,getUser,refreshSession,validToken,authFetch,requireAuth};
})();